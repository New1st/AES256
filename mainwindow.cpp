#include "mainwindow.h"
#include "ui_mainwindow.h"

MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::MainWindow)
{
    ui->setupUi(this);
    ui->counterLabel->setStyleSheet("QLabel { background-color : none; color : red; }");
    ui->decryptButton->setDisabled(true);
    ui->encryptButton->setDisabled(true);
}

MainWindow::~MainWindow()
{
    delete ui;
}

QString biteToQstring(QVector<unsigned char> bite)
{
    QByteArray bites;
    QString text;

    for(auto i:bite) bites.push_back(i);
    text = QString::fromLocal8Bit(bites);

    return text;
}

QVector<unsigned char> qstringToBite(QString text)
{
    QByteArray bites = text.toLocal8Bit();
    QVector<unsigned char> bite;

    for(auto i:bites) bite.push_back(i);

    return bite;
}

void MainWindow::on_encryptButton_clicked()
{
    QVector<unsigned char> bitesText = qstringToBite(ui->plainTextEdit->toPlainText());
    QVector<unsigned char> key = qstringToBite(ui->lineKeyEdit->text());
    QString bitesCode;
    array<unsigned char, 16> box;

    int n = 0;
    for(auto i:bitesText)
    {
        box[n++] = i;

        if(n == 16)
        {
            box = aes.encrypt(box, key);
            for(auto j:box)
            {
                bitesCode += QString::number(j) + " ";
            }
            n = 0;
        }
    }

    while(n != 0)
    {
        box[n++] = 0x00;

        if(n == 16)
        {
            box = aes.encrypt(box, key);
            for(auto j:box)
            {
                bitesCode += QString::number(j) + " ";
            }
            n = 0;
        }
    }

    ui->plainTextEdit->clear();
    ui->plainTextEdit->appendPlainText(bitesCode);
}

void MainWindow::on_decryptButton_clicked()
{
    QString bitesCode = ui->plainTextEdit->toPlainText();
    QVector<unsigned char> key = qstringToBite(ui->lineKeyEdit->text());
    QVector<unsigned char> bitesText;
    array<unsigned char, 16> box;

    int n = 0;
    QString s;
    for(auto i:bitesCode)
    {
        if(i == " ")
        {
            if(s != "") box[n++] = s.toInt();
            s = "";
        }
        else s = s + i;

        if(n == 16)
        {
            box = aes.decrypt(box, key);
            for(auto j:box) bitesText.push_back(j);
            n = 0;
        }
    }

    ui->plainTextEdit->clear();
    ui->plainTextEdit->appendPlainText(biteToQstring(bitesText));
}

void MainWindow::counterUpdate()
{
    if(ui->lineKeyEdit->text().length() < 16 )
    {
        ui->counterLabel->setStyleSheet("QLabel { background-color : none; color : red; }");
        ui->decryptButton->setDisabled(true);
        ui->encryptButton->setDisabled(true);
    }
    else
    {
        ui->counterLabel->setStyleSheet("QLabel { background-color : none; color : black; }");
        ui->decryptButton->setEnabled(true);
        ui->encryptButton->setEnabled(true);
    }
    ui->counterLabel->setText(QString::number(ui->lineKeyEdit->text().length()) + " / 16");
}
